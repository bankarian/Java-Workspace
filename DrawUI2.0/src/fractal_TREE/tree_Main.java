 ...
 * 
 */
public class tree_Main extends JWindow{
	private static final long serialVersionUID = 1L;
	public int animatingTime = 1;
	private static Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
	public static final int WIDTH = (int)screenSize.getWidth(), HEIGHT = (int)screenSize.getHeight();
	public static final int buttonW = 80;
	public String exit_button_address = "resource/final_exit.png";
	public static final int SLIDER_INIT = 250,SLIDER_MAX = SLIDER_INIT+50;
...
//		this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		...
		//setting up the container to store the components
		...
		class DrawPanel extends JPanel
		{
			...
			//Stopping condition for the recursion when len<=4
		...
		ImageIcon button = new ImageIcon(exit_button_address);
		JLabel buttonLabel = new JLabel(button);
		buttonLabel.setBounds(0, HEIGHT-buttonW, buttonW, buttonW);
		panel.add(buttonLabel);
		
		//setting the JSlider
		angleSlider = new JSlider(0, SLIDER_MAX, 0);					//最小值最大值,初始值
		...
		this.addMouseListener(new MouseAdapter(){
			...
			public void mouseClicked(MouseEvent e){
				...
				if(curC>=0 && curC<=buttonW && curR>=HEIGHT-buttonW && curR<HEIGHT){
					System.exit(0);
				}
			}
		});
		...
		new Timer(animatingTime,updateTask).start();										//延时参数,启动的动作监听
	}
